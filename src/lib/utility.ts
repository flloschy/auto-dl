...
export const sortWithLevenshtein = (list: string[], typed: string) =>
	list.sort((a, b) => levenshtein(a, typed) - levenshtein(b, typed));
...
export function cleanName(name: string) {
	return name.trim().replaceAll(' ', '_').replaceAll(removeNonAsciiRegex, '').slice(0, 20);
}
