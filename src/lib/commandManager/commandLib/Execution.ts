...
import { messageColors, type commandValueObject, type internalAsyncExecutionFunction, type logMessage } from "./UtilityTypes"
...
export class ExecutionHandler {
    ...
    log(value: logMessage, replace?: boolean) {
        this.logger.debug(`${replace ? "^ " : ""}${value.map(v => typeof v == "string" ? v : v.message).join(" ")}`)
