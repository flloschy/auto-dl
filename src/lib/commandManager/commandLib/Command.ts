...
import type { executionFunction, frontEndReadyCommand, frontEndReadySubCommand } from "./UtilityTypes"
...
export class InternalCommand {
    ...
    frontEndReadyObject():frontEndReadyCommand {
        ...
        return {
            ...
            options: Object.fromEntries(Object.entries(this.options).map(([key, option]) => ([key, option.frontEndReadyObject()]))),
            subCommands: Object.fromEntries(Object.entries(this.subCommands).map(([key, subCommand]) => ([key, subCommand.frontEndReadyObject()])))
        }
    }
}
...
export class CommandBuilder {
    ...
    addArgument(argument: ArgumentBuilder<any> | ArgumentBuilder<any>[]) {
        ...
        this.logger.debug("Adding argument: " + argument.internal.name + " [" + argument.internal.typeString + "]")
        // @ts-expect-error internal is private but still can be accessed after compiling
    ...
    addOption(option: OptionBuilder<any> | OptionBuilder<any>[]) {
        ...
        // @ts-expect-error internal is private but still can be accessed after compiling
        this.logger.debug("Adding option: " + option.internal.name + " [" + option.internal.typeString + "]")
        // @ts-expect-error internal is private but still can be accessed after compiling
