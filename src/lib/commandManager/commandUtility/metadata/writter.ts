...
import { readEpisodeNFO, readPlaylistNFO, readPodcastEpisodeNFO, readPodcastNFO, readSeasonNFO, readShowNFO } from "./reader";
...
export function writePlaylistNFO(playlist: string, data: Partial<AlbumNFO>) {
    ...
    setFile(`./downloads/music/${playlist}/album.nfo`, {album: {...oldData, ...cleanFlatObject(data)}})
}
...
export function writePodcastNFO(podcast: string, data: Partial<PodcastNFO>) {
    ...
    setFile(`./downloads/podcast/${podcast}/album.nfo`, {album: {...oldData, ...cleanFlatObject(data)}})
}
...
export function writeShowNFO(show: string, data: Partial<ShowNFO>) {
    ...
    setFile(`./downloads/youtube/${show}/tvshow.nfo`, {tvshow: {...oldData, ...cleanFlatObject(data)}})
}
...
export function writeSeasonNFO(show:string, season:string, data: Partial<SeasonNFO>) {
    ...
    setFile(`./downloads/youtube/${show}/${season}/season.nfo`, {season: {...oldData, ...cleanFlatObject(data)}})
}
export function writeEpisodeNFO(show: string, season: string, episode: string, data: Partial<EpisodeNFO>) {
    ...
    setFile(`${sections.path}/${sections.fileName}.nfo`, {episodedetails: {...oldData, ...cleanFlatObject(data)}})
}
export function writePodcastEpisodeNFO(podcast: string, episode: string, data: Partial<EpisodeNFO>) {
