...
export async function downloadYoutubeVideo(handler: ExecutionHandler, show: string, season: string, link: string) {
...
export async function downloadSpotifySong(handler: ExecutionHandler, playlist: string, spotify:string, youtube?:string) {
    handler.log(["downloading", {message: `${spotify}${youtube ? "|"+youtube : ""}`, color: messageColors.cyan}, "to", {message: playlist, color: messageColors.cyan}, "..."])
	...
	return await new Promise((resolve) => {
		...
		process.on('close', () => {
            handler.log([{message: "done", color: messageColors.green}, "downloading", {message: `${spotify}${youtube ? ":"+youtube : ""}`, color: messageColors.cyan}, "to", {message: playlist, color: messageColors.cyan}], true)
            resolve(true)
        });
		process.on('error', (e) => {
...
export async function downloadSpotifyPlaylist(handler: ExecutionHandler, playlist: string, spotify:string) {
    return await downloadSpotifySong(handler, playlist, spotify)
}


export async function downloadYoutubeSong<T extends "podcast"|"music">(handler: ExecutionHandler, playlist: string, link: string, type: T, infoJson: boolean = true) {
    ...
    const params = [
        ...
        '--quiet', // dont log anything else
        '-x', // extract audio
        "--audio-format", "mp3", // reformat to mp3
        `https://youtu.be/${id}`
    ].filter(x => x != "");
