...
export class PodcastEpisode {
	...
	constructor(podcast: Podcast, episode: string) {
		...
		this.metadata = {
			dateadded: NFODateFormatter(
				this.exists ? pathAge(`./downloads/podcast/${this.podcast.name}/${this.name}`) : 0
			),
			...
			episode: readdirSync(`./downloads/podcast/${this.podcast.name}`, { withFileTypes: true })
	...
	private load() {
		const segments = extractSectionsFromFile(
			`./downloads/podcast/${this.podcast.name}/${this.name}`
		);
