		...
		for (let index = 0; index < links.length; index++) {
			...
			if (link.includes('playlist')) {
				...
				for (let index2 = 0; index2 < ids.length; index2++) {
					const id = ids[index2];
					const out = await downloadYoutubeSong(
						handler,
						this.name,
						`https://youtu.be/${id}`,
						'podcast'
...
const podcastSelectArgumentNew = new ArgumentBuilder<Podcast>()
	...
	.setAutoComplete((pod) => [cleanName(pod.name)]);
...
export const podcastDownload = new SubCommandBuilder()
	...
	.addArgument(
		new ArgumentBuilder<string>()
			.setName('link')
			.setDescription(
				'the youtube url to download from (separate by comma), can be song or playlist url'
			)
			.setParser((v) => v.trim())
