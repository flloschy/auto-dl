...
import {
	validExistingFileName,
	validUrls
} from '$lib/commandManager/commandUtility/general/validators';
...
export class Album {
	...
	create() {
		...
		if (podcasts.includes(this.name))
			return {
				success: false,
	...
	edit(overwrite: Partial<AlbumNFO>) {
		...
		if (updatedName && overwrite.title) {
			renameSync(
				`./downloads/music/${this.name}`,
				`./downloads/music/${cleanName(overwrite.title)}`
			);
...
const albumSelectArgumentNew = new ArgumentBuilder<Album>()
	...
	.setAutoComplete((alb) => [cleanName(alb.name)]);
...
export const albumDownload = new SubCommandBuilder()
	...
	.addArgument(
		new ArgumentBuilder<string>()
			.setName('link')
			.setDescription(
				'spotify/youtube link or links separated by comma ("youtube|spotify" to download via youtube link and spotify metadata)'
			)
			.setParser((v) => v.trim())
...
export const songDelete = new SubCommandBuilder()
	...
	.setExecutionFunction<'instant'>((values) => {
		...
		try {
			rmSync(`${segments.path}/${segments.fileName}.lrc`);
		} catch {
			/*empty*/
		}
