import type { EpisodeNFO, PodcastEpisodeNFO } from "$lib/commandManager/commandUtility/metadata/types";
import { NFODateFormatter, clamp, cleanFlatObject, extractSectionsFromFile, getVideoIdFromYoutubeUrl, inRange, levenshtein, limitArrayLengthToX, pathAge, sortWithLevenshtein } from "$lib/utility";
...
export class Episode {
    ...
    private load() {
        const segments = extractSectionsFromFile(`./downloads/video/${this.show.name}/${this.season.name}/${this.name}`)
    ...
    async writeNfo() {
        this.metadata.runtime = await getDuration(`video/${this.show.name}/${this.season.name}/${this.name}`)
    ...
    delete() {
        ...
        const episodes = this.season.episodes().filter(e => parseInt(e.metadata.episode) > parseInt(this.metadata.episode))
