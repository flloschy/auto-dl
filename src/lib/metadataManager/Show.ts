...
export class Show {
	...
	edit(overwrite: Partial<ShowNFO>) {
		...
		if (updatedName && overwrite.title) {
			renameSync(
				`./downloads/video/${this.name}`,
				`./downloads/video/${cleanName(overwrite.title)}`
			);
...
const showSelectArgumentNew = new ArgumentBuilder<Show>()
	...
	.setAutoComplete((sho) => [cleanName(sho.name)]);
...
export const showCreate = new SubCommandBuilder()
	...
	.addOption([plotOption, releaseOption, studioOption, trailerOption, youtubemetadataidOption])
