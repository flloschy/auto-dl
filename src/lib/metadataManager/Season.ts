...
export class Season {
	...
	constructor(
		show: Show,
		loadOption: { name: string; folder?: undefined } | { folder: string; name?: undefined }
	) {
		this.show = show;
		...
		this.metadata = {
			dateadded: NFODateFormatter(
				this.exists ? pathAge(`./downloads/video/${this.show.name}/${this.name}`) : 0
			),
	...
	edit(overwrite: Partial<SeasonNFO>) {
		...
		if (overwrite.title && newTitle) {
			...
			renameSync(
				`./downloads/video/${this.show.name}/${this.name}`,
				`./downloads/video/${this.show.name}/${title}`
			);
			this.name = title;
		}
	...
	delete() {
		...
		const seasons = this.show
			.seasons()
			.filter((e) => parseInt(e.metadata.seasonnumber) > parseInt(this.metadata.seasonnumber));
	...
	episodes() {
		...
		const files = readdirSync(`./downloads/video/${this.show.name}/${this.name}`, {
			withFileTypes: true
		});
		const videos = files.filter(
			(v) => v.isFile() && !v.name.endsWith('nfo') && !v.name.endsWith('json')
		);
		...
		if (seasonnumberNew != undefined) {
            ...
            */
			...
			if (diff == 0) {
				...
				seasons
					.find((x) => parseInt(x.metadata.seasonnumber) == seasonnumberNew)
					?.edit({ seasonnumber: seasonnumberOld.toString() });
			} else {
				for (let index = 0; index < seasons.length; index++) {
