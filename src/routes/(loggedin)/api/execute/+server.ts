...
import type { commandValueObject, executionReturn } from "$lib/commandManager/commandLib/UtilityTypes.js";
...
export const POST = async ({ request }) => {
    ...
    if (subCommand != null) {
        internalSubCommand = Object.values(internalCommand.subCommands).find(cmd => cmd.name == subCommand)
    ...
    (executor.success ? logIt.info : logIt.error)(`Instant task completion ${executor.success ? "succeeded" : "failed"} (${executor.lines ? executor.lines.length : 0} lines output)`)
    return json({
        redirect: false,
        result: {
