<script lang="ts">
    ...
    import type {executionReturn, frontEndReadyCommand, frontEndReadySubCommand, logMessage} from "$lib/commandManager/commandLib/UtilityTypes";
    ...
    async function requestExecution() {
        ...
        if (activeSubCommand)
            body.append("subCommand", activeSubCommand.name)
...
<div class="grid">
    <CustomSelect
        ...
        descriptions={Object.values(data.commands).map(cmd => ({name: cmd.name, description: cmd.description}))}
        updateValue={(text) => {
    ...
    {#if activeBaseCommand != undefined && Object.keys(activeBaseCommand.subCommands).length != 0}
        <CustomSelect
            ...
            descriptions={Object.values(activeBaseCommand.subCommands).map(sub => ({name: sub.name, description: sub.description}))}
            updateValue={(text) => {
                if (!activeBaseCommand) return
